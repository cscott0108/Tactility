...
class DisplayIdleService final : public Service {
    ...
    void tick() {
        // Reload settings at the configured interval
        tickCount++;
#ifdef ESP_PLATFORM
        uint32_t reloadIntervalSeconds = service::development::getIdleCheckIntervalSeconds();
#else
        uint32_t reloadIntervalSeconds = 10; // Default for non-ESP platforms
#endif
        uint32_t ticksPerReload = (reloadIntervalSeconds * 1000) / 250; // 250ms per tick
        
        if (tickCount >= ticksPerReload) {
            reloadSettings();
            tickCount = 0;
        }
